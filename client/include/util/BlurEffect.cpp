...
BlurEffect::BlurEffect(IDirect3DDevice9* pDevice,
                       ...
                       const Resource& rEffect)
{
...

    {
        ...
        if (const HRESULT hResult = D3DXCreateEffect(pDevice, rEffect.GetDataPtr(), rEffect.GetDataSize(),
        ...
        {
            Logger::LogToFile("[err:blureffect_%p] : failed to create effect (code:%ld) (%.*s)",
                this, hResult, pErrorBuffer->GetBufferSize() - 1, pErrorBuffer->GetBufferPointer());
    ...
    {
        Logger::LogToFile("[err:blureffect_%p] : failed to create vertex declaration (code:%ld)", this, hResult);
        this->pEffect->Release();
        throw std::exception();
    }

    if (const HRESULT hResult = D3DXCreateTexture(pDevice, this->dParameters.BackBufferWidth,
        this->dParameters.BackBufferHeight, D3DX_DEFAULT, D3DUSAGE_RENDERTARGET,
        D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &this->pBackBufferTexture); FAILED(hResult))
    {
        Logger::LogToFile("[err:blureffect_%p] : failed to create back-buffer texture (code:%ld)", this, hResult);
        this->pEffect->Release();
    ...
    {
        Logger::LogToFile("[err:blureffect_%p] : failed to get back-buffer surface (code:%ld)", this, hResult);
        this->pEffect->Release();
        this->pVertexDeclaration->Release();
    ...
    if (const HRESULT hResult = D3DXCreateTexture(pDevice, this->dParameters.BackBufferWidth,
        this->dParameters.BackBufferHeight, D3DX_DEFAULT, D3DUSAGE_RENDERTARGET | D3DUSAGE_AUTOGENMIPMAP,
        D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &this->pTempBufferTexture); FAILED(hResult))
    {
        Logger::LogToFile("[err:blureffect_%p] : failed to create temp-buffer texture (code:%ld)", this, hResult);
        this->pEffect->Release();
        this->pVertexDeclaration->Release();
    ...
    {
        Logger::LogToFile("[err:blureffect_%p] : failed to get temp-buffer surface (code:%ld)", this, hResult);
        this->pEffect->Release();
        this->pVertexDeclaration->Release();
        this->pBackBufferSurface->Release();
        this->pBackBufferTexture->Release();
    ...
    {
        Logger::LogToFile("[err:blureffect_%p] : failed to create front-buffer texture (code:%ld)", this, hResult);
        this->pEffect->Release();
        this->pVertexDeclaration->Release();
        this->pBackBufferSurface->Release();
        this->pBackBufferTexture->Release();
    ...
    {
        Logger::LogToFile("[err:blureffect_%p] : failed to get front-buffer surface (code:%ld)", this, hResult);
        this->pEffect->Release();
        this->pVertexDeclaration->Release();
        this->pBackBufferSurface->Release();
        this->pBackBufferTexture->Release();
...
void BlurEffect::Render(float level) const noexcept
{
