...
void CSprite2d::Draw(float x, float y, float width, float height, float rx, float ry, CRGBA color, float angle, float hscale, float vscale)
{
	...
	ty1 = -ry * vscale;
	tx2 = (width - rx) * hscale;
	ty2 = (height - ry) * vscale;

	if (angle != 0.0f) {
		cost = -cosf(angle); sint = sinf(angle);
